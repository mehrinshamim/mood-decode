...
from app.schema.nlpschema import TextInput, MoodResponse, CrisisResponse, SummaryResponse
...
@router.post("/detect_crisis", response_model=CrisisResponse)
async def detect_crisis(input_data: TextInput):
    ...
    try:
        ...
        logger.error(f"Error detecting crisis: {str(e)}", exc_info=True)
